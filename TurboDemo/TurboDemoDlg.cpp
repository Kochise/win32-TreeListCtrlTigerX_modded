  ...
  : CDialog(CTurboDemoDlg::IDD, pParent)
{
  ...
  m_oStrTimeDelete = _T("- ms");
  m_oStrTimeInsert = _T("- ms");
  m_oStrTimeExpand = _T("- ms");
  //}}AFX_DATA_INIT
...
void CTurboDemoDlg::DoDataExchange(CDataExchange* pDX)
{
  ...
  DDX_Text(pDX, IDC_STATIC_DELETE, m_oStrTimeDelete);
  DDX_Text(pDX, IDC_STATIC_INSERT, m_oStrTimeInsert);
  DDX_Text(pDX, IDC_STATIC_EXPAND, m_oStrTimeExpand);
  //}}AFX_DATA_MAP
}
...
//    | TLC_HEADFULLDRAG    // head full drag
...
// MODIF_KOCH 060119
//  m_ctrlTreeList.Refresh(FALSE); // Block updates

  FeedTree();

//  m_ctrlTreeList.Refresh(TRUE); // Refresh
...
// MODIF_KOCH 060119
void CTurboDemoDlg::OnButtonRedo() 
{
  CWaitCursor l_oWaitCursor;

//  m_ctrlTreeList.Refresh(FALSE); // Block updates

  long tc_delete = GetTickCount();

  m_ctrlTreeList.DeleteAllItems();

  m_oStrTimeDelete.Format
  ( "%ld ms"
  , GetTickCount() - tc_delete
  );
...
void CTurboDemoDlg::FeedTree()
{
  long tc_insert = GetTickCount();
  ...
  m_oStrTimeInsert.Format
  ( "%ld ms"
  , GetTickCount() - tc_insert
  );

  long tc_expand = GetTickCount();
  m_ctrlTreeList.ExpandAll();
  m_oStrTimeExpand.Format
  ( "%ld ms"
  , GetTickCount() - tc_expand
  );
}
// MODIF_KOCH 060119
